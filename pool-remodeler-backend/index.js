...
const finishDetails = {
  ...
  tile: {
    ...
    prompt:
      "Transform the waterline and interior of this pool with National Pool Tile Blue Gemstone 6x6 ceramic tiles: glossy deep ocean-blue tiles arranged in a clean grid pattern along the waterline band and pool interior walls, rich sapphire-cobalt blue color, reflective glossy finish catching light. The pool deck stays the same. Photorealistic pool remodel, bright daylight, professional photography.",
  },
};

// Health check
app.get("/", (req, res) => res.json({ status: "ok", service: "AI Pool Remodeler API" }));
...
app.post("/remodel", upload.single("image"), async (req, res) => {
  try {
    ...
    const imageResponse = await openai.images.generate({
      model: "dall-e-3",
      prompt: `A professional pool remodeling transformation photo. ${finishInfo.prompt} Keep the same camera angle and pool shape as the original. Make it look like a real professional before/after remodel photo. High quality, photorealistic.`,
      n: 1,
      size: "1024x1024",
      quality: "standard",
    });
    ...
    // can embed it directly without any cross-origin issues
    ...
    console.error("Remodel error:", err);
    res.status(500).json({ error: err.message || "Something went wrong" });
  }
});

const PORT = process.env.PORT || 3001;
app.listen(PORT, () => console.log(`Pool Remodeler API running on port ${PORT}`));
